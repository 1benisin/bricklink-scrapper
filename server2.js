   ...
   try {
      ...
      for (const categoryCode of categoryCodes) {
         ...
         for (const partCode of partCodes) {
            colorCodes = colorCodes.concat(await getColorCodes(browser, partCode));

            // make csv file
            await fs.writeFile('sales.csv', `${DB_HEADERS}\n`);

            // scrape part price guide page
            for (const colorCode of colorCodes) {
               const detailsArr = await scrapePart(browser, partCode, colorCode);

               for (const e of detailsArr) {

                  //create a Part object
                  const part = Part.newPartfromArray(e);
...
Part.prototype.saveToDB = async function () {
   try {
      ...
      console.log('Saving to DB this: ', this.category_name, 'Part: ', this.part_code, 'Color: ', this.color_code, this.sales_country);
...
async function scrapePart(browser, part_code, color_code, options) {
   ...
   try {
      ...
      const details = await page.evaluate(() => {
         ...
         let partDetailsByCountry = [];
         for (let i = 0; i < countryNodes.length; i++) {
            // stat with empty array
            partDetailsByCountry = [];

            // get sales country name
            const sales_country = countryNodes[i].querySelector('center').innerText;

            // push everything except sales data onto details array
            partDetailsByCountry.push(category_name);
            partDetailsByCountry.push('part_code');
            partDetailsByCountry.push(part_name);
            partDetailsByCountry.push('color_code');
            partDetailsByCountry.push(color_name);
            partDetailsByCountry.push('bricklink_url');
