...
const linkProducts = `https://store.bricklink.com/ajax/clone/store/searchitems.ajax?pgSize=100&pg=1&showHomeItems=0&sid=${stores[curStore].id}`;
const linkStore = `https://store.bricklink.com/${stores[curStore].name}?p=${stores[curStore].name}#/shop?o={"pgSize":100,"pg":1,"showHomeItems":0}`;
...
async function waitUpTo(ms) {
  return new Promise((resolve) => {
    setTimeout(resolve, Math.floor(Math.random() * Math.floor(ms)));
  });
}
